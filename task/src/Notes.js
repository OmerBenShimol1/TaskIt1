...
const Notes = ({
    ...
    const handleEditNote = async (column, index, newTask, selectedColumn) => {
        try {
          const response = await axios.put('http://localhost:5001/taskit-backend/updateTaskData', {
      ...
      const handleDeleteNote = async (column, index, id) => {
        try {
          ...
          const response = await axios.delete(`http://localhost:5001/taskit-backend/deleteNote/${id}`);
